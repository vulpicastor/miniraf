...
SCALE_MAP = {"median": lambda a: a / np.median(a),
             "mean": lambda a: a / np.mean(a),
            }

def create_parser(subparsers):
    parser_combine = subparsers.add_parser("combine", help="combine help")
    parser_combine.add_argument("-m", "--method", choices=["median", "mean", "sum"], required=True)
    parser_combine.add_argument("-s", "--scale", choices=["median", "mean"])
